...
import { BookOpen, Terminal, Code2, Layers, Book, ShieldAlert, Sparkles, Rocket, Zap, Globe, MessageSquare, ArrowRight, Github } from 'lucide-react';
        ...
        <div className="flex flex-col gap-16 py-12">
            <header className="flex flex-col gap-6 max-w-2xl">
                <div className="flex items-center gap-3 px-3 py-1 bg-indigo-500/10 border border-indigo-500/20 rounded-full w-fit">
                    ...
                    <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-indigo-400">Knowledge Base</span>
                </div>
                <h1 className="text-5xl md:text-6xl font-black tracking-tighter text-white leading-none">
                    Build the future, <br />
                    <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 via-purple-400 to-cyan-400">faster than ever.</span>
                </h1>
                <p className="text-xl text-slate-400 font-light leading-relaxed">
                    Everything you need to build, deploy, and scale with Agentflox.
                    Explore our guides, API references, and architecture concepts.
                </p>
            </header>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {categories.map((cat, idx) => (
                    <Link
                        key={idx}
                        href={cat.href}
                        className="group relative flex flex-col p-8 bg-slate-900/30 border border-slate-800 rounded-3xl hover:border-indigo-500/30 transition-all hover:translate-y-[-4px] overflow-hidden"
                    >
                        ...
                        <h3 className="text-xl font-bold text-white mb-3 group-hover:text-indigo-400 transition-colors">{cat.title}</h3>
                        ...
                        <div className="mt-8 flex items-center gap-2 text-xs font-bold text-indigo-400/80 uppercase tracking-widest opacity-0 group-hover:opacity-100 transition-all translate-x-[-10px] group-hover:translate-x-0">
            ...
            <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-8">
                <div className="p-10 bg-gradient-to-br from-indigo-600/20 to-purple-600/20 border border-indigo-500/20 rounded-[2.5rem] flex flex-col items-start gap-4">
                    ...
                    <p className="text-slate-400">Agentflox is built on open standards and community feedback. Contribute to our documentation, share agents, and help us shape the future of work.</p>
                    <Link href="https://github.com/agentflox" className="flex items-center gap-2 px-6 py-3 bg-white text-black font-bold rounded-xl hover:bg-slate-200 transition-all mt-4">
                ...
                <div className="p-10 bg-slate-900 border border-slate-800 rounded-[2.5rem] flex flex-col items-start gap-4 shadow-2xl">
                    ...
                    <p className="text-slate-400">Our command-line tool offers an intuitive way to manage your agents and deployments directly from your terminal. Get started with zero configuration.</p>
