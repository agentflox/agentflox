...
const SidebarItem = ({ item, pathname }: { item: NavItem; pathname: string }) => {
    ...
    return (
        <Link
            href={item.href || '#'}
            className={`flex items-center gap-3 w-full px-3 py-2 text-sm font-medium rounded-lg transition-all ${isActive
        ...
        >
            <div className={`w-1.5 h-1.5 rounded-full transition-all ${isActive ? 'bg-indigo-400 shadow-[0_0_8px_rgba(99,102,241,1)]' : 'bg-slate-700'}`} />
            <span>{item.title}</span>
        </Link>
    );
};
