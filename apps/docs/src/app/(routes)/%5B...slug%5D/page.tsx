...
// Allow other slugs not listed above to still render on-demand
...
export default async function DocsPage({
  ...
  // Defensive guard: if slug is somehow undefined/empty during
  // build-time prerendering, fall back to an empty path instead
  // of crashing on `.join`.
    ...
    switch (slugValue) {
      ...
      default:
        return {
          ...
          description:
            "Deep dive into the architecture and capabilities of the Agentflox ecosystem.",
          content: `
    ...
    <div className="flex flex-col lg:flex-row gap-12 relative">
      <div className="flex-1 max-w-3xl">
        <nav className="flex items-center gap-2 mb-10 text-xs font-medium text-slate-500">
          ...
          <span className="text-indigo-400/80 uppercase tracking-wider truncate max-w-[200px]">
            {slugPath.split("/").join(" / ")}
          </span>
        </nav>
        ...
        <footer className="mt-24 pt-12 border-t border-slate-800/50">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-12">
            <Link
              href="#"
              className="p-6 bg-slate-900/30 border border-slate-800 rounded-xl hover:border-indigo-500/30 transition-all group flex flex-col items-start gap-1"
            >
            ...
            <Link
              ...
              className="p-6 bg-slate-900/30 border border-slate-800 rounded-xl hover:border-indigo-500/30 transition-all group flex flex-col items-end gap-1"
            >
              <span className="text-[10px] font-bold uppercase tracking-[0.2em] text-slate-500 group-hover:text-indigo-400 transition-colors">
                Next
              </span>
          ...
          <div className="mt-16 flex flex-col items-center justify-center gap-6 p-12 bg-indigo-500/5 border border-indigo-500/10 rounded-3xl text-center">
            ...
            <div className="flex items-center gap-4">
              <button className="px-6 py-2 bg-slate-900 border border-slate-800 rounded-lg text-sm font-medium hover:border-indigo-500/50 hover:bg-slate-800 transition-all">
                Yes, very!
              </button>
              <button className="px-6 py-2 bg-slate-900 border border-slate-800 rounded-lg text-sm font-medium hover:border-red-500/50 hover:bg-slate-800 transition-all">
                Not really
              </button>
            </div>
          </div>
          ...
          <div className="sticky top-32 flex flex-col gap-8">
            ...
            <div className="pt-8 border-t border-slate-800/50 flex flex-col gap-6">
              <div className="flex flex-col gap-2">
                ...
                <Link
                  href="#"
                  className="text-xs font-bold text-indigo-400 hover:underline mt-1"
                >
                  Join Workspace →
                </Link>
              </div>
