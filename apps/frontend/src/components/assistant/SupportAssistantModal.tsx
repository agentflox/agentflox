...
import { X, Minus, Sparkles, Headset } from "lucide-react";
...
export function SupportAssistantModal({ isOpen, onClose }: SupportAssistantModalProps) {
  ...
  useEffect(() => {
    ...
    initializeSupportAssistant("Support Assistant")
      .then((res) => {
  ...
  const handleSend = useCallback(async (content: string, options?: { modelId?: string }) => {
    ...
    try {
      await streamMessageToSupportAssistant(conversationId, content, {
        onToken: (text) => {
          setMessages((prev) =>
            prev.map((m) =>
        ...
        <>
          ...
          >
            ...
            {messages.map((msg) => (
              ...
              >
                {msg.role === "assistant" && (
                  <Avatar className="h-7 w-7 mt-1 border-2 border-white shadow-sm shrink-0">
                    <AvatarFallback className="bg-indigo-500 text-[10px] text-white font-bold">
                      AI
                    </AvatarFallback>
                  </Avatar>
                )}
