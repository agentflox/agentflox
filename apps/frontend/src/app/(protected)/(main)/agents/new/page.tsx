...
export default function AgentCreatePage() {
  ...
  const handleCardClick = async (card: Omit<AgentSuggestionCardProps | AgentTemplateCardProps, 'onClick' | 'disabled'>) => {
    ...
    try {
      ...
      // Step 4: Redirect to the agent detail page with build tab open
      router.push(`/agents/${agent.id}?tab=build`);
    } catch (error) {
  ...
  const handleGetStarted = async () => {
    ...
    try {
      ...
      // Redirect to the agent detail page with build tab open
      router.push(`/agents/${agent.id}?tab=build`);
    } catch (error) {
      console.error('Failed to create agent:', error);
      toast.error(error instanceof Error ? error.message : 'Failed to create agent');
