...
function WorkforceDetailContent() {
    ...
    const slugArray: string[] = React.useMemo(() => {
        ...
        const wfIdx = segments.indexOf('workforces');
        if (wfIdx !== -1 && segments.length > wfIdx + 2) {
            return segments.slice(wfIdx + 2);
        }
        return [];
    }, [params, pathname]);
    ...
    const resolvedTab = React.useMemo(() => {
        ...
        return tabParam === 'run' ? 'run' : 'build';
    }, [slugArray, tabParam]);

    const resolvedConvId = React.useMemo(() => {
        if (slugArray[0] === 'run') {
            if (slugArray[1] === 'chat' && slugArray[2]) return slugArray[2];
            if (slugArray[1] && slugArray[1] !== 'chat' && slugArray[1] !== 'log') return slugArray[1];
        }
        return conversationParam;
    }, [slugArray, conversationParam]);

    const [activeTab, setActiveTab] = React.useState<"build" | "run">(resolvedTab);
    const [runPaneReady, setRunPaneReady] = React.useState(() => resolvedTab === "run");
    const [buildPaneReady, setBuildPaneReady] = React.useState(() => resolvedTab !== "run");
    ...
    // Sync tab from URL only when it actually differs
    React.useEffect(() => {
        setActiveTab((prev) => (prev === resolvedTab ? prev : resolvedTab));
        if (resolvedTab === "run") setRunPaneReady(true);
        if (resolvedTab === "build") setBuildPaneReady(true);
    }, [resolvedTab]);

    // Sync conversationId from URL
    React.useEffect(() => {
        if (resolvedTab === "run") {
            setActiveConversationId((prev) => (prev === resolvedConvId ? prev : resolvedConvId));
            if (!resolvedConvId && params.id) {
                initConversationHandled.current = null;
            }
        }
    }, [resolvedConvId, resolvedTab, params.id]);
    ...
    React.useEffect(() => {
        ...
        if (first) {
            ...
            router.replace(`/workforces/${workforce.id}/run/chat/${first.id}`, { scroll: false });
        } else {
            ...
            createWorkforceConversation.mutateAsync({
                ...
                router.replace(`/workforces/${workforce.id}/run/chat/${conv.id}`, { scroll: false });
            }).catch((err) => {
                setPendingConversation(null);
                console.error("[Workforce] Failed to create conversation", err);
            });
        }
    }, [activeTab, workforce?.id, workforce?.name, workforce?.mode, name, workforceConversationsFetched, workforceConversations, resolvedConvId, router, createWorkforceConversation, utils]);
    ...
    const switchTab = (tab: "build" | "run") => {
        ...
        if (tab === "run") {
            router.push(activeConversationId ? `/workforces/${workforce.id}/run/chat/${activeConversationId}` : `/workforces/${workforce.id}/run`, { scroll: false });
        } else {
            router.push(`/workforces/${workforce.id}/build`, { scroll: false });
        }
                ...
                >
                    <WorkforceRunSidebar
                        ...
                        onSelectConversation={(id) => {
                            ...
                            router.replace(`/workforces/${workforce!.id}/run/chat/${id}`, { scroll: false });
                        }}
                        onNewTask={async () => {
                            const title = `${name || workforce!.name} – run`;
                            setPendingConversation({
                        ...
                        {mode === "FLOW" ? (
                            <WorkforceRunView
                                ...
                                onConversationReady={(conversationId) => {
                                    ...
                                    router.replace(`/workforces/${workforce!.id}/run/chat/${conversationId}`, { scroll: false });
                                }}
                            />
                        ) : (
                            <SwarmRunView
