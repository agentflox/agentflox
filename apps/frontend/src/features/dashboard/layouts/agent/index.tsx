    ...
    return (
        <div className="relative mx-auto w-full max-w-8xl h-full flex flex-col">
            ...
            <div className="hidden md:grid md:grid-cols-[var(--agent-sidebar-width,_16rem)_1fr] flex-1 min-h-0">
                <Sidebar />
                <div className={`${isFillHeightView ? 'h-full overflow-hidden flex flex-col min-h-0' : 'h-full overflow-y-auto'}`}>
                    {children}
                </div>
            </div>
            ...
            <div className="md:hidden flex-1 min-h-0 flex flex-col">
                <div className={`${isFillHeightView ? 'h-full overflow-hidden flex flex-col min-h-0' : 'p-4 h-full overflow-y-auto'}`}>
                    {children}
                </div>
                {/* Overlay sidebar */}
