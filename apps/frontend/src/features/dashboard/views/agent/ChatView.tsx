...
export const ChatView: React.FC<ChatViewProps> = ({
  ...
  const initializeBuilder = async (params: { agentId: string; conversationId?: string; skipWelcome?: boolean }) => {
    try {
      ...
      const res = await agentService.agents.executor.initialize(
        params as { conversationId?: string; agentId: string; skipWelcome?: boolean }
      );
      if (!res.ok) {
  ...
  useEffect(() => {
    ...
    if (resolvedAgentId) {
      ...
      console.log('[ChatView] No agent ID available, skipping initialization');
      setIsInitializing(false);
      return;
    }
