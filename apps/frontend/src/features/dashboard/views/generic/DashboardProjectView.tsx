...
export default function DashboardProjectView({ listId, spaceId, projectId, teamId, workspaceId, selectedTaskIdFromParent, onTaskSelect }: DashboardProjectViewProps) {
    ...
    const views = useMemo(() => {
        if (!projectData?.views) return [];
        return [...projectData.views].sort((a: any, b: any) => {
    ...
    const handleCopyViewLink = (view: any) => {
        const url = `${window.location.origin}${window.location.pathname}?pj=${projectId}&lv=${view.id}`;
                ...
                <div className="flex flex-col items-center gap-6 max-w-sm text-center">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <p className="text-sm text-slate-500 leading-relaxed">
                            This project doesn't exist or you don't have permission to access it. Check the URL or ask your workspace admin.
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-400">
                        ...
                        <span>Project ID: <code className="font-mono text-slate-500">{projectId ?? "—"}</code></span>
                    </div>
                </div>
            </div>
        );
                        ...
                        <TabsList className="h-auto bg-transparent p-0 flex-1 min-w-0 flex items-center overflow-visible">
                            <ViewTabsOverflow
                                ...
                                onReorderViews={(activeId, overId, dropPosition) => {
                                    ...
                                    if (projectId) {
                                        utils.project.get.setData({ id: projectId }, (old: any) => {
                                            if (!old) return old;
                                    ...
                                    <Popover modal={false}>
                                        ...
                                        <PopoverContent className="w-56 p-1" sideOffset={8} side="right" align="start">
                                            <div className="flex flex-col">
                                                ...
                                                <div className="h-px bg-slate-100 my-1 mx-2" />
                                                <div role="button" className="flex items-center gap-2 px-2 py-1.5 text-sm hover:bg-slate-100 rounded-sm text-slate-700 w-full text-left cursor-pointer transition-colors" onClick={(e) => { e.stopPropagation(); createViewMutation.mutate({ name: `${view.name} Copy`, type: view.type as any, projectId: projectId }); }}>
