...
export function TimelineView({ spaceId, projectId, teamId, listId, folderId, viewId, workspaceId, initialConfig, selectedTaskIdFromParent, onTaskSelect, refetchViewData }: TimelineViewProps) {
    ...
    const viewContentToSave = useMemo(() => {
        ...
        viewAutosave, zoomLevel, groupBy, groupDirection, sortBy, sort,
        showCompleted, showCompletedSubtasks, showTaskLocations, showWeekends,
        expandedSubtaskMode, filterGroups, defaultToMeMode, visibleColumns
    ]);
    ...
    return (
        <TooltipProvider>
            <div className="h-full flex flex-col bg-white hadow-sm overflow-hidden text-sm relative">
                {/* Toolbar Area */}
                <div className="flex items-center justify-between px-4 py-2 bg-white gap-4 z-50">
