...
export default function DashboardProjectView({ listId, spaceId, projectId, teamId, workspaceId, selectedTaskIdFromParent, onTaskSelect }: DashboardProjectViewProps) {
    ...
    const views = useMemo(() => {
        if (!spaceData?.views) return [];
        return [...spaceData.views].sort((a: any, b: any) => {
    ...
    const createViewMutation = trpc.view.create.useMutation({
        onSuccess: async () => {
            if (spaceId) await utils.space.get.invalidate({ id: spaceId });
        },
    ...
    const handleCopyViewLink = (view: any) => {
        const url = `${window.location.origin}${window.location.pathname}?sp=${spaceId}&lv=${view.id}`;
                ...
                <div className="flex flex-col items-center gap-6 max-w-sm text-center">
                    ...
                    <div className="space-y-1.5">
                        ...
                        <p className="text-sm text-slate-500 leading-relaxed">
                            This space doesn't exist or you don't have permission to access it. Check the URL or ask your workspace admin.
                        </p>
                    </div>
                    ...
                    <div className="flex items-center gap-2 text-xs text-slate-400">
                        ...
                        <span>Space ID: <code className="font-mono text-slate-500">{spaceId ?? "—"}</code></span>
                    </div>
                </div>
            </div>
        );
                        ...
                        <TabsList className="h-auto bg-transparent p-0 flex-1 min-w-0 flex items-center overflow-visible">
                            <ViewTabsOverflow
                                ...
                                onReorderViews={(activeId, overId, dropPosition) => {
                                    ...
                                    if (spaceId) {
                                        utils.space.get.setData({ id: spaceId }, (old: any) => {
                                            if (!old) return old;
                                    ...
                                    <Popover modal={false}>
                                        ...
                                        <PopoverContent className="w-56 p-1" sideOffset={8} side="right" align="start">
                                            <div className="flex flex-col">
                                                ...
                                                <div className="h-px bg-slate-100 my-1 mx-2" />
                                                <div role="button" className="flex items-center gap-2 px-2 py-1.5 text-sm hover:bg-slate-100 rounded-sm text-slate-700 w-full text-left cursor-pointer transition-colors" onClick={(e) => { e.stopPropagation(); createViewMutation.mutate({ name: `${view.name} Copy`, type: view.type, spaceId: spaceId }); }}>
