...
interface DashboardSpaceViewProps {
    listId?: string;
    spaceId?: string;
    projectId?: string;
    teamId?: string;
...
export default function DashboardSpaceView({ listId, spaceId, projectId, teamId, workspaceId, selectedTaskIdFromParent, onTaskSelect, context = "space" }: DashboardSpaceViewProps) {
    const { searchParams, parsedState } = useDashboardState();
    const router = useRouter();
    const utils = trpc.useUtils();

    const basePath = useMemo(() => {
        if (workspaceId && spaceId) return `/workspaces/${workspaceId}/sp/${spaceId}`;
    ...
    const views = useMemo(() => {
        ...
        const nonSidebarViews = spaceData.views.filter((v: any) => !v.sidebarView);
    ...
    const handleCopyViewLink = (view: any) => {
        const url = `${window.location.origin}${window.location.pathname}?sp=${spaceId}&v=${view.id}`;
