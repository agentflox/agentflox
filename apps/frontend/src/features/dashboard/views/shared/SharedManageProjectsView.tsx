...
import { Filter, MoreHorizontal, Eye, Trash, FolderKanban } from "lucide-react";
...
import { Settings2, ArrowUpDown, Check, ChevronUp, ChevronDown } from "lucide-react";
...
interface SharedManageProjectsViewProps {
    workspaceId?: string;
    spaceId?: string;
    teamId?: string;
...
export function SharedManageProjectsView({ workspaceId, spaceId, teamId, onProjectCreated }: SharedManageProjectsViewProps) {
    ...
    const handleBulkDelete = (rows: any[]) => console.log("Delete projects: ", rows.map(r => r.id));
    ...
    }, [router, onProjectCreated]);

    const columns: ColumnDef<any>[] = [
        {
            id: "select",
            header: ({ table }) => <Checkbox checked={table.getIsAllPageRowsSelected()} onCheckedChange={v => table.toggleAllPageRowsSelected(!!v)} aria-label="Select all" className="translate-y-[2px]" />,
            cell: ({ row }) => <Checkbox checked={row.getIsSelected()} onCheckedChange={v => row.toggleSelected(!!v)} aria-label="Select row" className="translate-y-[2px]" />,
            enableSorting: false,
            enableHiding: false,
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return (
                    <div className="flex flex-col">
                        <span className="font-medium text-foreground hover:underline cursor-pointer" onClick={() => handleOpen(project.id)}>
                            {project.title || "Untitled Project"}
                        </span>
                        {project.description && <span className="text-xs text-muted-foreground truncate max-w-[250px]">{project.description}</span>}
                    </div>
                );
            },
        },
        ...
        },
        {
            ...
            cell: ({ row }) => {
                ...
                return <span className="text-sm text-muted-foreground">{formatDistanceToNow(new Date(row.original.updatedAt), { addSuffix: true })}</span>;
            },
        },
        {
            id: "actions",
                ...
                return (
                    <DropdownMenu>
                        ...
                        <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => handleOpen(project.id)}><Eye className="mr-2 h-4 w-4" />View</DropdownMenuItem>
                            <DropdownMenuSeparator />
                            <DropdownMenuItem className="text-destructive focus:text-destructive" onClick={() => console.log("Delete", project.id)}><Trash className="mr-2 h-4 w-4" />Delete</DropdownMenuItem>
                        </DropdownMenuContent>
                    </DropdownMenu>
                );
            },
    ...
    return (
        <div className="h-full overflow-y-auto p-6 bg-zinc-50/50">
            <div className="space-y-6 max-w-7xl mx-auto">
                <PageHeader
                    title="Projects"
                    description="Manage your projects across your organization."
                    actions={
                        <Button
                            onClick={() => setShowCreateModal(true)}
                            className="group flex items-center gap-2 bg-zinc-900 hover:bg-zinc-700 text-white px-4 py-2 h-9 rounded-md transition-all duration-300 shadow-sm hover:shadow-md active:scale-[0.98]"
                        >
                ...
                >
                    <DropdownMenu>
                        <DropdownMenuTrigger asChild>
                            <Button variant="ghost" className="h-9 px-3 gap-2 text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100/80 transition-all">
                                ...
                                <span>Filter</span>
                                {(scope !== "all" || (filters as any).status) && (
                                    <span className="ml-1 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-zinc-200/70 px-1.5 text-xs font-semibold text-zinc-700">
                                        {(scope !== "all" ? 1 : 0) + ((filters as any).status ? 1 : 0)}
                                    </span>
                                )}
                            </Button>
                        </DropdownMenuTrigger>
                    ...
                    {viewMode === "list" && table && (() => {
                        const hideableColumns = table.getAllColumns().filter(c => typeof c.accessorFn !== "undefined" && c.getCanHide() && c.id !== "title");
                        ...
                        return (
                            <DropdownMenu>
                                ...
                                <DropdownMenuContent align="end" className="w-[160px]">
                                    <DropdownMenuCheckboxItem checked={true} onCheckedChange={() => { }} className="opacity-50 cursor-not-allowed">Project title</DropdownMenuCheckboxItem>
                                    <DropdownMenuSeparator />
                    ...
                    <Popover>
                        <PopoverTrigger asChild>
                            <Button variant="ghost" className="h-9 gap-1.5 px-3 text-zinc-600 hover:text-zinc-900 hover:bg-zinc-100/80 transition-all cursor-pointer rounded-md focus:ring-0 focus-visible:ring-0">
                                <ArrowUpDown className="h-4 w-4" />
                                <span>Sort</span>
                                {sort.length > 0 && <span className="ml-1 flex h-5 min-w-[20px] items-center justify-center rounded-full bg-zinc-200/70 px-1.5 text-xs font-semibold text-zinc-700">{sort.length}</span>}
                            </Button>
                        </PopoverTrigger>
                        <PopoverContent align="end" className="w-[240px] p-1.5 rounded-xl shadow-xl border-zinc-200" sideOffset={8}>
                            <div className="px-2 py-1.5 mb-1"><span className="text-[11px] font-bold text-zinc-400 uppercase tracking-widest">Sort By</span></div>
                            <div className="space-y-0.5">
                                <div className="flex items-center gap-2.5 px-2 py-1.5 text-sm rounded-md cursor-pointer hover:bg-zinc-50 text-zinc-600" onClick={() => setSort([])}>
                                    <span className="flex-1">None (default)</span>{sort.length === 0 && <Check className="h-3.5 w-3.5 text-zinc-900" />}
                                </div>
                                {[{ id: "title", label: "Title" }, { id: "status", label: "Status" }, { id: "updatedAt", label: "Updated Date" }].map(opt => {
                                    const idx = sort.findIndex(s => s.id === opt.id);
                                    const isSelected = idx >= 0;
                                    const cur = isSelected ? sort[idx] : null;
                                    return (
                                        <div key={opt.id}
                                        ...
                                        >
                                            <div className="h-5 w-5 flex items-center justify-center rounded hover:bg-zinc-200" onClick={e => { e.stopPropagation(); if (isSelected) setSort(s => s.map(i => i.id === opt.id ? { ...i, desc: !i.desc } : i)); else setSort(s => [...s, { id: opt.id, desc: false }]); }}>
                                                {isSelected && <div className="flex flex-col items-center -space-y-1"><ChevronUp className={`h-3.5 w-3.5 ${cur?.desc ? 'text-zinc-800' : 'text-zinc-300'}`} /><ChevronDown className={`h-3.5 w-3.5 ${cur?.desc ? 'text-zinc-300' : 'text-zinc-800'}`} /></div>}
                                            </div>
                ...
                {chips.length > 0 && (
                    <div className="flex flex-wrap items-center gap-2">
                        {chips.map(chip => (
                            <button key={chip.id} onClick={chip.onRemove} className="group inline-flex items-center gap-2 rounded-md border border-zinc-200 bg-white pl-2.5 pr-2 py-1 text-xs font-medium text-zinc-700 hover:bg-zinc-50 hover:border-zinc-300 cursor-pointer">
                                <span>{chip.label}</span>
                                <div className="flex items-center justify-center rounded-full p-0.5 group-hover:bg-zinc-200/60"><X className="h-3 w-3 text-zinc-400 group-hover:text-zinc-700" /></div>
                            </button>
                        ))}
                        <Button variant="ghost" onClick={clearAll} className="h-7 px-2 text-xs text-zinc-500 hover:text-zinc-900">Clear all</Button>
                    </div>
                )}
                ...
                {isLoading ? (
                        ...
                        <>
                            <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
                                {data.items.map((p: any) => (
                                    <ProjectCard key={p.id} item={p} onOpen={handleOpen} isSelected={selectedGridIds.has(p.id)} onSelect={handleGridSelect} />
                                ))}
                            </div>
                            {selectedGridIds.size > 0 && (
                                <div className="fixed bottom-6 left-1/2 z-50 -translate-x-1/2 flex items-center gap-4 rounded-xl border border-zinc-200 bg-white/90 px-5 py-3 shadow-2xl backdrop-blur-md ring-1 ring-zinc-100 animate-in fade-in slide-in-from-bottom-4 duration-200">
                                    ...
                                    <Button variant="destructive" size="sm" onClick={() => { console.log("Delete:", [...selectedGridIds]); setSelectedGridIds(new Set()); }} className="h-8 gap-1.5 px-3 cursor-pointer"><Trash className="h-3.5 w-3.5" />Delete Selected</Button>
                                </div>
                            )}
                        </>
                    ) : (
                    ...
                    <div className="flex min-h-[400px] flex-col items-center justify-center rounded-2xl border border-slate-200/80 bg-gradient-to-b from-slate-50/50 to-white shadow-sm">
                        <div className="text-center px-6 py-8 max-w-xs">
                            ...
                            <h3 className="text-base font-semibold text-slate-900">{query ? "No results found" : "No projects yet"}</h3>
                            <p className="mt-2 text-sm text-slate-500 leading-relaxed">
                                {query ? "Try adjusting your search or clearing filters." : "Create and manage your projects to start organizing your work."}
                            </p>
                            {!query && (
