...
import {
    ...
    Square, Check, Folder, SquareArrowRight, LogOut,
    HelpCircle, Hourglass, Target, CheckCircle2,
    MessageSquare, MinusCircle, GripVertical, Pencil, LayoutGrid, List as ListIcon, ArrowLeftRight, FileText,
    AlertTriangle, Link2, Bell, SlidersHorizontal, ThumbsUp, SmilePlus, Reply, Store
} from 'lucide-react';
...
import { TaskPermissionsModal } from '@/entities/task/components/TaskPermissionsModal';
import { TaskMoveAndAddPopover } from '@/entities/task/components/TaskMoveAndAddPopover';
...
import { TimeTrackingModal } from '@/entities/task/components/TimeTrackingModal';
import { TaskLinksPanelContent } from '@/entities/task/components/TaskLinksPanelContent';
import { LinksPanelContent } from '@/entities/task/components/LinksPanelContent';
import { AddRelationshipSection } from '@/entities/task/components/AddRelationshipSection';
import type { CustomRelationshipType } from '@/entities/task/components/CreateCustomRelationshipModal';
import { ChecklistsSection } from '@/entities/task/components/ChecklistsSection';
import { SubtasksTable } from '@/entities/task/components/SubtasksTable';
import { AttachmentsSection } from '@/entities/task/components/AttachmentsSection';
import { BlockingPanelContent } from '@/entities/task/components/BlockingPanelContent';
import { WaitingOnPanelContent } from '@/entities/task/components/WaitingOnPanelContent';
import { DocLinksPanelContent } from '@/entities/task/components/DocLinksPanelContent';
...
interface TaskDetailViewProps {
     ...
     * In a view context this typically navigates away or clears the selected task.
     */
    onClose: () => void;
 ...
 * TaskDetailView
 *
 * Renders the full task detail UI as a plain view — no Dialog, no backdrop,
 * no modal behaviour. Drop it anywhere in your layout:
 *
 *   <TaskDetailView taskId={selectedId} onClose={() => setSelectedId(null)} />
 *
 * The root element is a flex column that fills its parent (`h-full w-full`).
 * Wrap it in whatever container suits your page (e.g. a split-pane right panel,
 * a drawer, a dedicated route page, etc.).
 */
export function TaskDetailView({
    ...
    const [rightSidebarPanel, setRightSidebarPanel] = React.useState<
        'activity' | 'blocking' | 'waitingOn' | 'links' | 'taskLinks' | 'docLinks' | 'related' | 'more' | null
    >('activity');
    const [customRelationshipTypes, setCustomRelationshipTypes] = React.useState<CustomRelationshipType[]>([]);
    const [activityFilterOpen, setActivityFilterOpen] = React.useState(false);
    const [activityFilterTypes, setActivityFilterTypes] = React.useState<Set<string>>(
        () => new Set(ACTIVITY_FILTER_OPTIONS.map((o) => o.id))
    );
        ...
        onMutate: async (variables) => {
            ...
            utils.task.get.setData({ id: taskId }, (old: any) => {
                ...
                const updateTaskInTree = (t: any): any => {
                    if (t.id === variables.id) return { ...t, ...variables };
                    if (t.other_tasks) {
                        return { ...t, other_tasks: t.other_tasks.map((st: any) => updateTaskInTree(st)) };
                    }
                    return t;
                };
    ...
    if (isLoading) {
        return (
            <div className={cn('h-full flex items-center justify-center bg-white', className)}>
    ...
    return (
        <div className={cn('flex flex-col h-full w-full bg-white', className)}>
            {/* ── Top Bar ──────────────────────────────────────────────────── */}
            <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-200 bg-white z-20 shrink-0">
                {/* Left side */}
                    ...
                    <div className="flex items-center gap-1.5 text-xs text-zinc-600 min-w-0 flex-1">
                        {task.space && (
                            <>
                                ...
                                <span className="hover:underline cursor-pointer truncate">{task.space.name}</span>
                        ...
                        {task.list && (
                            <>
                                ...
                                <span className="hover:underline cursor-pointer font-medium text-zinc-700 truncate">{task.list.name}</span>
                                <ChevronRight className="h-3 w-3 text-zinc-300 shrink-0" />
                            </>
                        )}
                ...
                <div className="flex items-center gap-2.5">
                    ...
                    <TaskActionsDropdown
                        ...
                        tooltip="Settings"
                        trigger={
                            <Button size="icon" variant="ghost" className="h-8 w-8 text-zinc-500 hover:text-zinc-700 p-2">
                                    ...
                                    <ScrollArea className="flex-1">
                                        <div className="p-2 space-y-1">
                                            {subtasks.map((subtask: any) => (
                                                <div key={subtask.id} className="p-2 rounded-md hover:bg-white cursor-pointer group transition-colors">
                                                    <div className="flex items-start gap-2">
                                                        ...
                                                        <div className="flex-1 min-w-0">
                                                            <div className="text-xs text-zinc-700 font-medium truncate">{subtask.title}</div>
                                                            {subtask.dueDate && (
                                            ...
                                            {subtasks.length === 0 && (
                                                <div className="text-center py-8 text-xs text-zinc-400">No subtasks yet</div>
                                            )}
                                        </div>
                                    </ScrollArea>
                            ...
                            <Group id="task-detail-left-right" orientation="horizontal" className="flex-1 min-w-0">
                                <Panel defaultSize={65} minSize={35}>
                                    <div className="flex flex-col h-full min-w-0 overflow-hidden">
                                        ...
                                        <div className="flex border-b border-zinc-200 bg-white shrink-0">
                                            {(['details', 'subtasks', 'action-items'] as const).map((tab) => (
                                                <button
                                                    key={tab}
                                                    type="button"
                                        ...
                                        <div className="flex-1 overflow-y-auto min-h-0">
                                            {/* ── DETAILS TAB ── */}
                                            {leftTab === 'details' && (
                                                <div className="pl-6 pr-0 py-6 space-y-8 w-full">
                                                    {/* Title */}
                                                    {isEditingTitle ? (
                                                    ...
                                                    <div className="grid grid-cols-2 md:grid-cols-4 gap-y-6 gap-x-8">
                                                                ...
                                                                <DropdownMenu>
                                                                    <DropdownMenuTrigger asChild>
                                                                        <div className="flex items-center gap-2 group cursor-pointer hover:bg-zinc-50 p-1 -ml-1 rounded transition-colors">
                                                                            <div className="h-2.5 w-2.5 rounded-full ring-2 ring-white shadow-sm" style={{ backgroundColor: task.status?.color || '#ccc' }} />
                                                                            <span className="text-sm font-medium text-zinc-700">{task.status?.name || 'To Do'}</span>
                                                                        </div>
                                                                    </DropdownMenuTrigger>
                                                                    <DropdownMenuContent align="start">
                                                                        {(task.list?.statuses ?? []).map((s: any) => (
                                                                            <DropdownMenuItem key={s.id} onClick={() => updateTask.mutate({ id: task.id, statusId: s.id })}>
                                                                                <div className="flex items-center gap-2">
                                                                                    <div className="h-2 w-2 rounded-full shrink-0" style={{ backgroundColor: s.color || '#94A3B8' }} />
                                                                                    <span>{s.name}</span>
                                                                                </div>
                                                                            </DropdownMenuItem>
                                                                        ))}
                                                        ...
                                                        <div className="space-y-1.5">
                                                            ...
                                                            <div className="flex items-start">
                                                                <AssigneeSelector
                                                                    ...
                                                                    onChange={(newIds) => updateTask.mutate({ id: task.id, assigneeIds: newIds })}
                                                                    variant="compact"
                                                                    trigger={
                                                            ...
                                                            <DropdownMenu>
                                                                <DropdownMenuTrigger asChild>
                                                                    <div className="flex items-center gap-2 text-sm font-medium text-zinc-700 cursor-pointer hover:bg-zinc-50 p-1 -ml-1 rounded transition-colors">
                                                                        {task.priority !== 'NORMAL' ? (
                                                                            <>
                                                                                <Flag className={cn('h-3.5 w-3.5 fill-current',
                                                                ...
                                                                <DropdownMenuContent align="start">
                                                                    {[
                                                                        ...
                                                                        >
                                                                            <div className="flex items-center gap-2">
                                                                                <Flag className={cn('h-3 w-3 fill-current', priority.color)} />
                                                            ...
                                                            <Popover>
                                                                ...
                                                                <PopoverContent className="w-auto p-0" align="start">
                                                                    <TaskCalendar
                                                                        ...
                                                                        onStartDateChange={(date) => updateTask.mutate({ id: task.id, startDate: date ? date.toISOString() : null })}
                                                                        onEndDateChange={(date) => updateTask.mutate({ id: task.id, dueDate: date ? date.toISOString() : null })}
                                                                    />
                                                                </PopoverContent>
                                                            </Popover>
                                                        </div>
                                                        ...
                                                        <div className="space-y-1.5">
                                                            ...
                                                            <div className="flex flex-wrap gap-1.5 min-h-[28px] items-center">
                                                                ...
                                                                {customRelationshipTypes.map((ct) => (
                                                                    <button
                                                                        key={ct.id}
                                                                        type="button"
                                                                        onClick={() => setRightSidebarPanel('related')}
                                                    ...
                                                    <div className="space-y-3">
                                                        ...
                                                        <CustomFieldsSection taskId={task.id} workspaceId={task.workspaceId} />
                                                    </div>
                                            ...
                                            {leftTab === 'action-items' && (
                                                <div className="px-8 py-6">
                                                    ...
                                                    <ChecklistsSection taskId={task.id} workspaceMembers={workspaceMembers} />
                                                </div>
                                            )}
                                        </div>
                                    </div>
                                ...
                                <Panel defaultSize={35} minSize={25}>
                                    <div className="flex border-l border-zinc-200 bg-white h-full min-h-0 w-full min-w-0">
                                        {/* Panel content */}
                                        {rightSidebarPanel && (
                                            <div className="flex-1 min-w-[280px] flex flex-col min-h-0 bg-white border-r border-zinc-100 overflow-hidden">
                                                {/* Activity */}
                                                                ...
                                                                <Popover open={activityFilterOpen} onOpenChange={setActivityFilterOpen}>
                                                                    ...
                                                                    <PopoverContent className="w-72 p-0" align="end">
                                                                        <div className="flex items-center justify-between border-b border-zinc-100 px-3 py-2">
                                                                            ...
                                                                            <button type="button" className="text-xs text-zinc-500 hover:text-zinc-700" onClick={() => setActivityFilterTypes(new Set())}>
                                                                                Unselect All
                                                                            </button>
                                                                        </div>
                                                                        <ScrollArea className="max-h-[280px]">
                                                                            <div className="p-2 space-y-0.5">
                                                                                {ACTIVITY_FILTER_OPTIONS.map((opt) => {
                                                                                    ...
                                                                                    return (
                                                                                        <label key={opt.id} className="flex items-center gap-2 py-2 px-2 rounded-md hover:bg-zinc-50 cursor-pointer">
                                                                                            <Checkbox
                                                                                                checked={checked}
                                                                                                onCheckedChange={(c) => {
                                                                                    ...
                                                                                    <div className="text-zinc-600">
                                                                                        {item.type === 'comment' ? (
                                                                                            ...
                                                                                            <>
                                                                                                ...
                                                                                                {' '}{
                                                                                                    item.action === 'CREATED' ? 'created the task' :
                                                                                                        item.action === 'ASSIGNED' ? 'assigned' :
                                                                                                            item.action === 'UNASSIGNED' ? 'unassigned' :
                                                                                                                item.action === 'STATUS_CHANGED' ? 'updated status' :
                                                                                                                    item.action === 'PRIORITY_CHANGED' ? 'updated priority' :
                                                                                                                        item.action === 'DUE_DATE_CHANGED' ? 'updated due date' :
                                                                                                                            item.action === 'ATTACHED' ? 'added an attachment' :
                                                                                                                                item.action === 'COMMENTED' ? 'commented' :
                                                                                                                                    item.action === 'MOVED' ? 'moved' : 'updated'
                                                                                                }{' '}{item.field ? (item.field === 'title' ? 'name' : item.field === 'statusId' ? 'status' : item.field === 'listId' ? 'list' : item.field) : 'the task'}
                                                                                            </>
                                                                                        )}
                                                                                    </div>
