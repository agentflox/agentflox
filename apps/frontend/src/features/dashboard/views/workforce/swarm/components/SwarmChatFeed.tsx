...
export interface ConvMessage {
  ...
  ts: number;
  meta?: { mentions?: any[]; contexts?: any[] } | null;
}
    ...
    <div className="border-l-4 border-amber-500 bg-amber-50 rounded-r-xl shadow-inner px-4 py-3 w-full">
      ...
      <div className="flex gap-2 mt-2.5">
        <button
          ...
          className="flex-1 flex items-center justify-center gap-1.5 py-1.5 bg-amber-500 hover:bg-amber-600 active:bg-amber-700 text-white text-[11px] font-bold rounded-lg transition-colors shadow-sm"
        >
          <Check className="h-3.5 w-3.5" /> Approve
        </button>
        <button
          onClick={() => onDeny(task.id)}
          className="flex-1 flex items-center justify-center gap-1.5 py-1.5 bg-white hover:bg-red-50 active:bg-red-100 text-red-600 text-[11px] font-bold rounded-lg border border-red-200 transition-colors"
        >
      ...
      {hasResult && (
        <div className="mt-2 relative">
          ...
          <button
            ...
            className="mt-1 flex items-center gap-1 text-[10px] text-emerald-600 hover:text-emerald-800 font-semibold transition-colors"
          >
...
export function InterAgentMessage({ payload, time }: { payload: any; time: string }) {
  ...
  return (
    <div className="ml-6 border-l-2 border-teal-200 pl-3">
      <div className="bg-teal-50 rounded-xl rounded-bl-none px-3 py-2 text-xs">
        <div className="flex items-center justify-between mb-1">
          <span className="font-bold text-[10px] text-teal-700 uppercase tracking-wide">
            {sender} <span className="text-teal-400">→</span> {receiver}
          </span>
          <span className="text-[9px] text-teal-400">{time}</span>
        </div>
        <p className="text-teal-900 whitespace-pre-wrap text-[11px] leading-relaxed">{payload.detail || payload.content}</p>
      </div>
    </div>
  );
}
          ...
          if (msg.swarmEvent) {
            ...
            if (type === 'SESSION_STARTED') return (
              <div key={item.id} className="flex items-center gap-3 py-1">
                ...
                <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-indigo-50 border border-indigo-200 text-[11px] font-bold text-indigo-700">
                  <Zap className="h-3 w-3" />{payload.label || '🚀 Swarm started'}
                  <span className="text-indigo-400 font-normal ml-1">{time}</span>
                </div>
                <div className="flex-1 h-px bg-indigo-100" />
              </div>
            );
            ...
            if (type === 'SESSION_STOPPED') return (
              <div key={item.id} className="flex items-center gap-3 py-1">
                ...
                <div className="flex items-center gap-1.5 px-3 py-1 rounded-full bg-red-50 border border-red-200 text-[11px] font-bold text-red-600">
                  <XCircle className="h-3 w-3" />{payload.label || '🛑 Session stopped'}
                  <span className="text-red-300 font-normal ml-1">{time}</span>
                </div>
                <div className="flex-1 h-px bg-red-100" />
              </div>
            );
            ...
            if (type === 'COORDINATOR_INSPECT') return (
              <div key={item.id} className="flex items-start gap-2">
                <div className="h-7 w-7 rounded-full bg-violet-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <Zap className="h-3.5 w-3.5 text-violet-600" />
                </div>
                <div className="bg-violet-50 border border-violet-100 rounded-2xl rounded-tl-sm px-3 py-2 text-xs text-violet-800 flex-1 max-w-[90%]">
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-[10px] text-violet-600 uppercase tracking-wide">Coordinator</span>
                    <span className="text-[9px] text-violet-300">{time}</span>
                  </div>
                  <p className="font-semibold text-violet-900 mb-1">{payload.label}</p>
                  {payload.detail && <pre className="text-violet-700 whitespace-pre-wrap text-[10.5px] leading-relaxed">{payload.detail}</pre>}
                </div>
              </div>
            );

            if (type === 'COORDINATOR_ASSIGN') return (
              <div key={item.id} className="flex items-start gap-2">
                <div className="h-7 w-7 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0 mt-0.5">
                  <CheckCircle2 className="h-3.5 w-3.5 text-blue-600" />
                </div>
                <div className="bg-blue-50 border border-blue-100 rounded-2xl rounded-tl-sm px-3 py-2 text-xs flex-1 max-w-[90%]">
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-[10px] text-blue-600 uppercase tracking-wide">Task Assignment</span>
                    <span className="text-[9px] text-blue-300">{time}</span>
                  </div>
                  <p className="font-semibold text-blue-900 mb-0.5">{payload.taskTitle}</p>
                  <p className="text-blue-600">{payload.detail}</p>
                </div>
              </div>
            );

            if (type === 'AGENT_PROGRESS') return (
              <div key={item.id} className="flex items-start gap-2">
                ...
                <div className="bg-amber-50 border border-amber-100 rounded-2xl rounded-tl-sm px-3 py-2 text-xs flex-1 max-w-[90%]">
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-[10px] text-amber-600 uppercase tracking-wide">Agent {payload.from?.slice(0, 8) ?? ''}</span>
                    <span className="text-[9px] text-amber-300">{time}</span>
                  </div>
                  <p className="text-amber-900">{payload.detail}</p>
                </div>
              </div>
            );
            ...
            if (type === 'TASK_COMPLETED') return (
              <div key={item.id} className="w-full">
                <TaskResultCard payload={payload} time={time} />
              </div>
            );
            ...
            if (type === 'TASK_FAILED') return (
              <div key={item.id} className="flex items-start gap-2">
                ...
                <div className="border-l-4 border-red-500 bg-red-50 rounded-r-xl px-3 py-2.5 text-xs flex-1">
                  <div className="flex items-center justify-between mb-1">
                    <span className="font-bold text-[10px] text-red-600 uppercase tracking-widest">Task Failed</span>
            ...
            return (
              <div key={item.id} className="flex items-start gap-2">
                ...
                <div className="bg-zinc-50 border border-zinc-100 rounded-2xl rounded-tl-sm px-3 py-2 text-xs text-zinc-600 max-w-[85%]">
                  <span className="font-bold text-[10px] text-zinc-400 block mb-0.5">{type}</span>
                  {msg.content}
                  <span className="text-[9px] text-zinc-300 ml-2">{time}</span>
                </div>
              </div>
            );
          }
              ...
              {msg.role === "user" ? (
                <>
                  <div className="flex-1 rounded-2xl rounded-tl-sm bg-white border border-zinc-200 px-4 py-3 shadow-sm flex flex-col items-start min-w-0">
                    {editingIndex === idx ? (
                      <textarea defaultValue={msg.content} autoFocus
                        className="w-full min-h-[60px] text-sm text-zinc-900 resize-none outline-none"
                        onBlur={e => { setMessages(p => p.map((m, i) => i === idx ? { ...m, content: e.target.value } : m)); setEditingIndex(null); }}
                    ...
                    {msg.meta && (msg.meta.mentions?.length > 0 || msg.meta.contexts?.length > 0) && editingIndex !== idx && (
                  ...
                  {editingIndex !== idx && (
                    <button onClick={() => setEditingIndex(idx)} className="opacity-0 group-hover:opacity-100 p-1.5 rounded hover:bg-zinc-100 text-zinc-400 transition-opacity">
        ...
        <div className="flex items-start justify-end">
          <div className="rounded-2xl bg-zinc-900 text-zinc-50 px-4 py-3 text-sm max-w-[90%]">
            <div className="flex items-center gap-2 text-zinc-300">
              ...
              {pollingExecutionId ? `Running (${pollingExecutionId.slice(0, 8)}…) — finalizing…` : "Finalizing…"}
            </div>
          </div>
        </div>
      )}
