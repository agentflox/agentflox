...
import type { RunRecord, RunLogEntry } from "../hooks/useToolRun";

export type ToolRunViewProps = {
          ...
          {selectedRun ? (
            <>
              ...
              {selectedRun.logs.map((log, i) => (
                <div key={i} className={`rounded-lg px-4 py-2.5 flex gap-3 items-start ${
                  log.type === "thinking" ? "bg-zinc-900" :
                  log.type === "error" ? "bg-red-950" :
                  log.type === "complete" ? "bg-emerald-950" :
                  "bg-zinc-800"
                }`}>
                  <span className={`shrink-0 text-[10px] font-bold uppercase mt-0.5 w-14 ${
                    log.type === "thinking" ? "text-indigo-400" :
