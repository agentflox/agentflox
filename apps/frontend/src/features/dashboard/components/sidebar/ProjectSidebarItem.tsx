...
export function ProjectSidebarItem({
    ...
    // Fetch child folders under this project
    const { data: foldersData } = trpc.folder.byContext.useQuery(
        { projectId: project.id, directOnly: true, includeViewDetails: false },
        { enabled: true }
    );

    // Fetch child lists under this project (direct-only)
    const { data: listsData } = trpc.list.byContext.useQuery(
        { projectId: project.id, directOnly: true, includeViewDetails: false },
        { enabled: true }
    );

    // Fetch child docs under this project (direct-only)
    const { data: docViewsData } = trpc.view.list.useQuery(
        { projectId: project.id, directOnly: true, type: "DOC", sidebarView: true },
        { enabled: true }
    );
        ...
        <div className="relative select-none">
            ...
            >
                ...
                >
                    ...
                    <span
                        className={cn(
                            ...
                            hasChildren && "group-hover/project:hidden"
                        )}
                        style={{ backgroundColor: project.icon ? (project.color || "#6366f1") : "transparent" }}
                    >
                        <ProjectIcon
                            icon={project.icon}
                            className={cn(project.icon ? "text-white" : isActive ? "text-indigo-500" : "text-indigo-500/80")}
                            size={14}
                            fill
                        />
                    </span>
                    ...
                    {hasChildren && (
                        <div className="hidden group-hover/project:flex items-center justify-center h-5 w-5 rounded bg-zinc-200 text-zinc-700 hover:bg-zinc-300 transition-colors">
                ...
                <span className={cn(
                    ...
                    isActive ? "font-normal text-foreground" : "text-zinc-600 group-hover/project:text-foreground"
                )}>
                    {project.name}
                </span>
