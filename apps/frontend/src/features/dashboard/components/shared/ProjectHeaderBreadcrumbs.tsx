...
export function ProjectHeaderBreadcrumbs({
    ...
    const currentSubItem = useMemo(() => {
        ...
        return null;
    }, [isTeamTab, activeTeam, activeList, activeFolder, isDocsTab, isChannelsTab, isAiChatTab, isPersonalTab, activeChannel, activeAiChat, selectedChatId, selectedAiChatId, currentTab]);
    ...
    const handleSaveSubName = () => {
        ...
        if (currentSubItem.type === "team" && activeTeam) {
    ...
    const handleCopySubLink = () => {
        ...
        if (currentSubItem.type === "team" && activeTeam) {
            url = `${window.location.origin}${window.location.pathname}?projectId=${projectId}&team=${activeTeam.id}`;
        } else if (currentSubItem.type === "list" && activeList) {
    ...
    const handleNavigate = (type: "team" | "folder" | "list" | "channels" | "ai-chat", id: string) => {
        setSubPopoverOpen(false);
        if (type === "team") {
            if (onSelectTeam) onSelectTeam(id);
            else router.push(`?projectId=${projectId}&tm=${id}`, { scroll: false });
        } else if (type === "folder") {
    ...
    const SubIcon = currentSubItem?.icon || ListIcon;
        ...
        <div className="flex items-center gap-0.5 min-w-0">
            ...
            {currentSubItem && (
                <>
                    ...
                    {["team", "list", "folder", "channels", "ai-chat"].includes(currentSubItem.type) ? (
                            ...
                            >
                                ...
                                <div className="flex items-center gap-1.5 pb-1 border-b border-slate-100">
                                    ...
                                    <Input
                                        ...
                                        onKeyDown={(e) => e.key === "Enter" && e.currentTarget.blur()}
                                        disabled={!["team", "list", "folder"].includes(currentSubItem.type)}
                                ...
                                {currentSubItem.type === "channels" ? (
                                    ...
                                    /* Full Nested Hierarchy Tree under current Project */
                                    <div className="flex flex-col gap-0.5 max-h-72 overflow-y-auto pr-0.5 select-none">
