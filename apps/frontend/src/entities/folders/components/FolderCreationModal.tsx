...
export function FolderCreationModal({ context, contextId, workspaceId, parentFolderId, open: controlledOpen, onOpenChange: setControlledOpen, trigger, onFolderCreated }: FolderCreationModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      let initialKey = "";
      if (parentFolderId) initialKey = `FOLDER:${parentFolderId}`;
  ...
  }, [isOpen, context, contextId, parentFolderId, workspaceId]);
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!form.name.trim()) {
      toast({ title: "Missing details", description: "Please provide a folder name.", variant: "destructive" });
      return;
    }

    if (!form.destinationKey) {
      toast({ title: "Missing destination", description: "Please select a location for the folder.", variant: "destructive" });
      return;
    }
    ...
    try {
      ...
      toast({ title: "Could not create the folder", description: error?.message ?? "Please try again.", variant: "destructive" });
    }
  }
        ...
        <DialogTrigger asChild>
          {trigger || (
            <Button className="max-w-24" variant="outline">
              <FolderIconLucide className="mr-2 h-4 w-4" />
              Create Folder
            </Button>
          )}
        </DialogTrigger>
      ...
      <DialogContent className="sm:max-w-xl p-0 overflow-hidden gap-0 border-border/50 shadow-2xl bg-background/95 backdrop-blur-xl transition-all duration-300">
        <div className="p-6 pb-2">
          <div className="flex items-start gap-5">
            <div className={cn(
              ...
              <FolderIconLucide className="w-5 h-5 md:w-6 md:h-6" strokeWidth={1.5} />
            </div>
            <div className="pt-1">
              ...
              <DialogDescription className="text-muted-foreground text-sm leading-relaxed">
                Organize your lists into a folder.
              </DialogDescription>
            </div>
          </div>
        </div>
        ...
        <form className="flex flex-col" onSubmit={handleSubmit}>
          <div className="px-6 py-6 space-y-6">
            ...
            <div className="space-y-2">
              <Label htmlFor="folder-name" className="text-sm font-medium text-zinc-700">
                ...
                <Input
                  ...
                  onChange={(event) => {
                    ...
                    setForm((prev) => ({
                      ...prev,
                      name: newName,
                      ...(!prev.hasManualIcon && { icon: newName.trim().charAt(0).toUpperCase() || "F" })
                    }));
                  }}
            ...
            <div className="space-y-0">
              <div className="flex items-center justify-between">
                <Label htmlFor="folder-description" className="text-sm font-medium text-zinc-700">
              ...
              <Textarea
                ...
                name="description"
                placeholder="Tell us a bit about your Folder..."
