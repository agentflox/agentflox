...
interface FolderTransferModalProps {
...
export function FolderTransferModal({ folderId, folderName, open, onOpenChange, onSuccess }: FolderTransferModalProps) {
    ...
    const {
        ...
        {
            enabled: !!folderId && open && debouncedEmail.length >= 3,
            staleTime: 30_000,
            retry: false,
        }
    );
    ...
    const handleConfirmTransfer = async () => {
        if (!folderId || !selectedUser) return;

        setIsTransferring(true);
        try {
            const res = await fetch(`/api/permissions/folder/${folderId}/transfer`, {
                method: "POST",
            ...
            toast({
                ...
                description: `${displayName(selectedUser)} is now the owner of ${folderName}`,
            });
            ...
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-zinc-500 leading-relaxed pr-4">
                        This will transfer{" "}
                        <strong className="font-semibold text-zinc-700">"{folderName}"</strong> full
                        control of the folder to the new owner, and you'll be downgraded to Admin.{" "}
