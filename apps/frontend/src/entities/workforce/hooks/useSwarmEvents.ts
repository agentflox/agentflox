...
import { useWorkforceStore } from './useWorkforceStore';
...
export function useSwarmEvents() {
    ...
    useEffect(() => {
        ...
        (async () => {
            ...
            try {
                ...
                const res = await fetch(`${BACKEND_URL}/v1/workforces/swarm/${swarmSessionId}/events`, {
    ...
    const fetchBacklog = async () => {
        try {
            const token = await fetchAuthToken();
            const resp = await fetch(`${BACKEND_URL}/v1/workforces/swarm/tasks?status=PENDING,OPEN,IN_PROGRESS&sessionId=${swarmSessionId}`, {
                headers: token ? { Authorization: `Bearer ${token}` } : {},
            });
            if (resp.ok) {
