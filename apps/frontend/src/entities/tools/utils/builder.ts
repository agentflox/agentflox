...
import type { InputUiType, BuilderInputField } from "../types/builder";

export function branchEntryIcon(nodeType: string) {
  if (nodeType === "inputs") return React.createElement(Files, { className: "h-3.5 w-3.5 text-sky-500" });
  if (nodeType === "tool") return React.createElement(Wrench, { className: "h-3.5 w-3.5 text-emerald-500" });
  return React.createElement(Bot, { className: "h-3.5 w-3.5 text-violet-500" });
}
...
export function branchTypeIcon(t: string) {
  if (t === "Number") return React.createElement("span", { className: "text-[9px] font-black text-sky-600" }, "#");
  return React.createElement("span", { className: "text-[9px] font-black text-amber-600" }, "T");
}
