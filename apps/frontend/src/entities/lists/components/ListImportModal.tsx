...
import { Loader2, List as ListIcon, Building2, CheckCircle2 } from "lucide-react";
...
export function ListImportModal({ spaceId, open, onOpenChange }: ListImportModalProps) {
    ...
    // Assuming we fetch unassigned lists here. In reality backend may need a specific query for this
    const { data, isLoading } = trpc.list.byContext.useQuery({
        workspaceId: undefined, // Usually you'd pass a workspace ID to find all unassigned lists in it
    ...
    const handleImport = async () => {
        ...
        try {
            // TODO: Backend does not explicitly support spaceId in list update yet
            /*
            await Promise.all(selectedIds.map(id =>
                updateList.mutateAsync({ id, spaceId: selectedSpaceId })
            ));
            */
            toast({ title: `${selectedIds.length} list${selectedIds.length > 1 ? "s" : ""} imported (Mocked)` });
    ...
    const toggleAll = () =>
        setSelectedIds(allSelected ? [] : lists.map(l => l.id));
            ...
            <DialogContent className="sm:max-w-[480px] p-0 gap-0 overflow-hidden">
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <DialogDescription className="text-xs text-slate-500 mt-0.5 leading-relaxed">
                                Move unassigned lists into a workspace space to organize them.
                            </DialogDescription>
                        </div>
                    </div>
                </div>
                                ...
                                {isLoading ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center gap-2 h-52 p-4">
                                        ...
                                        <div className="text-center">
                                            <p className="text-xs font-medium text-slate-700">All lists are assigned</p>
                                            <p className="text-xs text-slate-400 mt-0.5">No unassigned lists to import</p>
                                        </div>
                                    </div>
                                ) : (
                ...
                <DialogFooter className="px-6 py-4 bg-slate-50/80 border-t border-slate-100 flex items-center gap-2">
                    ...
                    <Button
                        ...
                        disabled={selectedIds.length === 0 || updateList.isPending}
                        className={cn(
                    ...
                    >
                        {updateList.isPending && <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" />}
                        Import {selectedIds.length > 0 ? selectedIds.length : ""} list{selectedIds.length !== 1 ? "s" : ""}
                    </Button>
                </DialogFooter>
            </DialogContent>
        </Dialog>
