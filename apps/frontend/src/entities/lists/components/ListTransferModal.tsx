...
export function ListTransferModal({ listId, listName, open, onOpenChange, onSuccess }: ListTransferModalProps) {
    ...
    const {
        ...
        {
            enabled: !!listId && open && debouncedEmail.length >= 3,
            staleTime: 30_000,
            retry: false,
        }
    );
    ...
    const handleConfirmTransfer = async () => {
        ...
        try {
            const res = await fetch(`/api/permissions/list/${listId}/transfer`, {
                method: "POST",
            ...
            toast({
                ...
                description: `${displayName(selectedUser)} is now the owner of ${listName}`,
            });
            ...
            <DialogContent className="sm:max-w-md">
                <DialogHeader>
                    ...
                    <DialogDescription className="text-zinc-500 leading-relaxed pr-4">
                        This will transfer{" "}
                        <strong className="font-semibold text-zinc-700">"{listName}"</strong> full
                        control of the list to the new owner, and you'll be downgraded to Admin.{" "}
