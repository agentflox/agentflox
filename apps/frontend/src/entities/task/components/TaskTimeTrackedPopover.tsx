...
/** Parse "3h 20m", "1h", "45m", "90" etc. to total seconds. */
...
/** Format seconds → "3h 20m" or "45m" or "Add time". */
function formatDurationShort(seconds: number): string {
    if (seconds <= 0) return 'Add time';
...
interface TaskTimeTrackedPopoverProps {
    taskId: string;
    workspaceId?: string | null;
    /** Pre-computed total from the task._count or parent — used before the popover is opened */
    totalTrackedSeconds?: number;
    trigger?: React.ReactNode;
}

export function TaskTimeTrackedPopover({
    taskId,
    workspaceId,
    totalTrackedSeconds: externalTotal = 0,
    trigger,
}: TaskTimeTrackedPopoverProps) {
    ...
    const currentUser = session
        ? { id: session.id, name: session.name ?? session.email ?? 'You', image: session.image }
        : null;
    ...
    const workspaceMembers = React.useMemo(() => {
        const fromWs = (workspaceData?.members ?? []).map((m: any) => ({
            id: m.user.id,
            name: m.user.name ?? m.user.email ?? 'Unknown',
            image: m.user.image,
        }));
        if (currentUser && !fromWs.some((m: any) => m.id === currentUser.id)) {
    ...
    const displayUser = React.useMemo(() => {
        ...
        return workspaceMembers.find((m: any) => m.id === id) ?? currentUser;
    ...
    const displayTotal = open
        ? (isTimerRunning ? totalTracked + timerSeconds : totalTracked)
        : externalTotal;
    const displayWithout = open
        ? (isTimerRunning ? totalTracked + timerSeconds : totalTracked)
        : externalTotal;

    const entriesByUser = React.useMemo(() => {
        const map = new Map<string, { user: any; entries: typeof timeEntries }>();
        for (const e of timeEntries) {
            const uid = e.user?.id ?? 'unknown';
            if (!map.has(uid)) map.set(uid, { user: e.user ?? { id: uid }, entries: [] });
    ...
    // Label shown in the cell
    const cellLabel = open
        ? formatDurationShort(displayTotal)
        : formatDurationShort(externalTotal);
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            ...
            <PopoverContent
                ...
                sideOffset={12}
                collisionPadding={20}
                className="w-[520px] p-0 rounded-xl shadow-2xl border border-zinc-200 bg-white overflow-y-auto max-h-[85vh]"
                onClick={(e) => e.stopPropagation()}
            >
                <div className="p-4 space-y-4">
                    ...
                    <div>
                        <div className="flex items-center justify-between">
                            <h2 className="text-base font-semibold text-zinc-900">Time on all tasks</h2>
                    ...
                    <div className="rounded-lg border border-zinc-200 bg-zinc-50/30 p-3 space-y-3">
                        ...
                        {displayUser && (
                            <Popover>
                                <PopoverTrigger asChild>
                                    <button
                                        type="button"
                                        className="flex items-center gap-2 rounded-full bg-zinc-900 text-white pl-1 pr-2 py-1 text-sm font-medium hover:bg-zinc-800 transition-colors cursor-pointer"
                                    >
                                    ...
                                    <div className="max-h-[240px] overflow-y-auto space-y-0.5">
                                        {workspaceMembers.map((m: any) => (
                                            <button
                                                key={m.id}
                                                type="button"
                                                className="w-full flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-zinc-100 text-left cursor-pointer"
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <Button
                                ...
                                className={cn(
                                    ...
                                    isTimerRunning ? 'bg-red-500 hover:bg-red-600' : 'bg-zinc-400 hover:bg-zinc-500'
                                )}
                                onClick={() => {
                        ...
                        <div className="flex items-center gap-2 text-sm text-zinc-600">
                            ...
                            <Popover>
                                <PopoverTrigger asChild>
                                    <button type="button" className="hover:underline text-left cursor-pointer">
                        ...
                        <div className="flex items-center gap-2 text-sm text-zinc-600">
                            ...
                            <span className="text-zinc-500 shrink-0">Add tags</span>
                            <Input
                                value={tagsInput}
                                                ...
                                                >
                                                    <span className="text-zinc-600 truncate flex-1 min-w-0">
                                                        {entry.startTime && entry.endTime
                                                            ? `${format(new Date(entry.startTime), 'EEE, MMM d, h:mm a')} – ${format(new Date(entry.endTime), 'h:mm a')}`
