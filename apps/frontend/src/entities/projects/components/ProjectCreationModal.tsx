	...
	const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
		...
		if (!form.destinationKey) {
			toast({
				...
				description: "Please select a location for the project.",
				variant: "destructive",
			});
			return;
		}
		...
		try {
			...
			dispatch(upsertProject({ id: createdId, data: serializeDates(data as any) }));
				...
				<form className="flex flex-col" onSubmit={handleSubmit}>
					<div className="px-6 py-6 space-y-6">
						...
						<div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
							...
							<div className="space-y-2">
								<Label htmlFor="project-visibility" className="text-sm font-medium text-zinc-700">
									Visibility
								</Label>
								<Select
								...
								>
									<SelectTrigger id="project-visibility" className="w-full rounded-md shadow-none bg-white border-slate-200 hover:border-slate-300 hover:bg-zinc-50">
					...
					<div className="px-6 py-4 bg-muted/20 flex flex-wrap items-center justify-end gap-3 border-t border-border/40">
						<UsageRemainingHint kind="PROJECT" className="mr-auto w-full sm:w-auto" />
						<Button
							type="button"
							variant="ghost"
