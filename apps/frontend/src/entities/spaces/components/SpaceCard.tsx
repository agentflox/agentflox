...
import { MoreVertical, Clock, Users, Folder } from "lucide-react";
import {
	DropdownMenu,
	DropdownMenuContent,
	DropdownMenuItem,
				...
				<div className="flex flex-wrap items-center gap-4 text-xs font-medium text-zinc-400 dark:text-zinc-500 mt-2">
					{item._count && (
						<>
							...
							{((item._count as any).folders !== undefined || item._count.lists !== undefined) && (
								<div className="flex items-center gap-1.5">
                                    <Folder className="h-4 w-4 text-zinc-300" />
									<span className="text-zinc-600 dark:text-zinc-300">
										{(item._count as any).folders ?? item._count.lists}
									</span>
								</div>
							)}
						</>
