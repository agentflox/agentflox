				...
				{TASK_IMPORT_ITEMS_COL1.map((item) => (
					<div key={item.id} className="flex items-center space-x-3">
						...
						<label
							...
							className="text-[13.5px] font-normal cursor-pointer text-slate-700 leading-none"
						>
							{item.label}
						</label>
					</div>
...
function ImportOptionsPanel({
	...
	return (
		<div className="pt-2">
			<label className="text-[13px] font-semibold text-slate-700 block mb-2">Import options</label>
			<div className="rounded-lg border border-slate-200 bg-white overflow-hidden">
				{/* Tabs Wrapper */}
				<div className="bg-slate-50/80 p-1.5 flex gap-1 border-b border-slate-200">
					<button
						type="button"
											...
											<div className="flex items-start gap-4">
												...
												<div className="flex-1 min-w-0">
													<label
														...
														className="text-[14px] font-medium text-slate-700 cursor-pointer"
													>
														{toggle.label}
													</label>
													{toggle.description && (
														<p className="text-[12px] text-slate-400 mt-0.5">{toggle.description}</p>
													)}
			...
			<div className="max-w-[640px] mx-auto space-y-8">
				...
				<div>
					...
					<div className="flex items-center gap-4 text-xs font-medium text-slate-500 pt-3">
						{!showDescription && (
							<button onClick={() => setShowDescription(true)} className="hover:text-slate-700 transition-colors cursor-pointer">+ Add description</button>
						)}
						{!showTags && (
							<button onClick={() => setShowTags(true)} className="hover:text-slate-700 transition-colors cursor-pointer">+ Add tags</button>
						)}
					</div>
				</div>
				...
				{showDescription && (
					<div>
						<div className="flex items-center justify-between mb-2">
							<label className="text-[13px] font-semibold text-slate-700 block">Description</label>
							<button onClick={() => { setShowDescription(false); setTemplateDescription(""); }} className="text-slate-400 hover:text-slate-600 cursor-pointer">
								<X className="size-3.5" />
							</button>
						</div>
						<textarea
							placeholder="Enter description..."
							className="w-full flex min-h-[80px] rounded-md border border-slate-200 bg-white px-3 py-2 text-[14px] shadow-sm transition-colors focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-indigo-500 focus-visible:border-indigo-500 placeholder:text-slate-400 placeholder:font-normal resize-y"
				...
				{showTags && (
					<div>
						<div className="flex items-center justify-between mb-2">
							<label className="text-[13px] font-semibold text-slate-700 block">Tags</label>
							<button onClick={() => { setShowTags(false); setTemplateTags([]); }} className="text-red-400 hover:text-red-500 cursor-pointer">
								<Trash2 className="size-3.5" />
							</button>
						</div>
						<div className="min-h-10 rounded-md border border-slate-200 bg-white px-3 py-2 shadow-sm flex items-center flex-wrap gap-2">
							{templateTags.length > 0 && templateTags.map((t, idx) => (
								<div key={idx} className="flex items-center gap-1.5 px-2 py-[2px] bg-slate-100 rounded text-xs text-slate-600 border border-slate-200">
							...
							<TagsModal
								...
								trigger={
									<button className="flex size-7 items-center justify-center border border-slate-200 hover:bg-slate-50 transition-colors rounded-md text-slate-500 cursor-pointer shadow-sm">
				...
				{/* Share With */}
				<div className="pt-2">
					<label className="text-[13px] font-semibold text-slate-700 block mb-2">Share with</label>
					<div className="border rounded-md border-slate-200">
												...
												<div className="relative inline-flex items-center">
														...
														return (
															<>
																{visibleAvatars.map((invitee, idx) => (
																	...
																	>
																		<Avatar className={cn(
																			"h-7 w-7 border-2 border-white ring-1 ring-slate-100",
																		...
																		{invitee.type === 'team' && (
																			<div className="absolute -bottom-1 -right-1 h-3.5 w-3.5 bg-slate-100 rounded-full flex items-center justify-center ring-1 ring-white">
																				<Users className="h-[8px] w-[8px] text-slate-500" />
																			</div>
																		)}
																	</div>
																))}
																{remainingCount > 0 && (
																	<div className="relative shrink-0 -ml-2" style={{ zIndex: 50 - maxAvatars }}>
																		<div className="h-7 w-7 rounded-full bg-slate-100 border-2 border-white ring-1 ring-slate-100 flex items-center justify-center text-[10px] font-bold text-slate-600">
																			{remainingCount}+
																		</div>
																	</div>
																)}
													...
													<Popover open={invitePopoverOpen} onOpenChange={setInvitePopoverOpen}>
														<PopoverTrigger asChild>
															<button 
																className="-ml-2 h-7 w-7 rounded-full bg-white flex flex-shrink-0 items-center justify-center text-slate-400 border border-slate-200 border-dashed hover:border-slate-300 hover:text-slate-600 transition-colors cursor-pointer"
																style={{ zIndex: 10 }}
															>
														...
														<PopoverContent className="w-[300px] p-0" align="start" onClick={(e) => e.stopPropagation()}>
															<div className="p-2 border-b border-slate-100">
																<div className="flex flex-1 h-8 items-center rounded-md border border-slate-200 bg-white px-2 shadow-sm focus-within:border-indigo-500">
																	<Search className="h-3.5 w-3.5 shrink-0 text-slate-400" />
																	<input
																		type="text"
																		placeholder="Search or enter email..."
																		className="flex-1 h-full bg-transparent pl-2 pr-0 text-xs outline-none border-none placeholder:text-slate-400"
																		value={inviteSearch}
															...
															<ScrollArea className="max-h-[250px]">
																<div className="py-2">
																	...
																	<div className="px-3 py-1 text-xs font-semibold text-slate-500">People</div>
																		...
																		<>
																			{filteredUsers.map(member => {
																				...
																				return (
																					<button
																						key={member.user.id}
																						className={cn("w-full px-3 py-2 cursor-pointer flex items-center justify-between text-left group", isSelected ? "bg-slate-50" : "hover:bg-slate-50")}
																						onClick={(e) => {
																							e.preventDefault();
																							if (isSelected) {
																					...
																					>
																						<div className="flex items-center gap-3">
																							...
																							<span className={cn("text-sm truncate", isSelected ? "text-indigo-950 font-medium" : "text-slate-700")}>{member.user.name}</span>
																						</div>
																						{isSelected && (
																							<div className="text-[10px] font-medium px-2 py-0.5 rounded border border-slate-200 bg-white text-slate-500 shadow-sm">Profile</div>
																						)}
																					</button>
																				)
																			})}
																	...
																	{(filteredTeams.length > 0) && (
																		<>
																			...
																			<div className="px-3 py-1 text-xs font-semibold text-slate-500">Teams</div>
																			{filteredTeams.map(team => {
																				...
																				return (
																					...
																					>
																						<div className="flex items-center gap-3">
																							<div className="relative">
																								<div className={cn("h-7 w-7 rounded-full bg-slate-400 text-white flex items-center justify-center", isSelected && "ring-2 ring-indigo-600 ring-offset-1")}>
																							...
																							<span className={cn("text-sm truncate", isSelected ? "text-indigo-950 font-medium" : "text-slate-700")}>{team.name}</span>
																						</div>
																						{!isSelected && <span className="text-xs text-slate-400">{(team as any)?._count?.members || 0} people</span>}
																					</button>
																				)
																			})}
																		</>
																	...
																	{inviteSearch && (
																		<button className="w-full mt-1 px-3 py-2.5 mx-2 bg-slate-50 hover:bg-slate-100 rounded-md cursor-pointer flex items-center gap-3 text-left w-[calc(100%-16px)]">
																			<UserPlus className="h-4 w-4 text-slate-600" />
																			<span className="text-sm font-medium text-slate-700">Invite people via email</span>
																		</button>
																	)}
																</div>
															</ScrollArea>
						...
						<div className="p-4 border-t border-slate-200 flex items-start gap-4">
							<Switch
								id="public-share"
							...
							/>
							<div>
								<Label htmlFor="public-share" className="text-[14px] text-slate-600 cursor-pointer block">
									Public sharing
								</Label>
								<p className="text-xs text-slate-400 mt-1">Save to create public link.</p>
							</div>
						</div>
					</div>
				</div>
	...
	if (view === "update-select") {
		return (
			<Dialog open={open} onOpenChange={onOpenChange}>
				...
				>
					<div className="flex items-center justify-between px-6 py-4 border-b border-slate-100 shrink-0 relative">
						<button
							onClick={() => {
								if (previousView) {
						...
						<DialogTitle className="text-[15px] font-semibold text-slate-800 absolute left-1/2 -translate-x-1/2">
					...
					<div className="flex-1 overflow-y-auto px-10 py-8 bg-white">
						{showUpdateEmptyState ? (
							<div className="h-full flex flex-col items-center justify-center text-center">
								<div className="relative size-14 rounded-2xl border border-slate-300/80 bg-white flex items-center justify-center mb-4">
									<Wand2 className="size-7 text-slate-300" />
									<div className="absolute -right-1 -bottom-1 size-5 rounded-full bg-slate-900 text-white flex items-center justify-center shadow-sm">
										<Search className="size-2.5" />
									</div>
								</div>
								<p className="text-[21px] font-semibold text-slate-800">No templates found</p>
								<p className="mt-2 text-[14px] text-slate-500">Create a template in your Workspace first!</p>
								<Button
							...
							<div className="max-w-[600px] mx-auto space-y-4">
								<div className="flex items-center gap-3">
									<div className="flex flex-1 overflow-hidden h-10 items-center rounded-md border border-slate-200 bg-white px-3 shadow-sm transition-colors focus-within:ring-1 focus-within:ring-indigo-500 focus-within:border-indigo-500">
										<Search className="h-4 w-4 shrink-0 text-slate-400" />
										<Input
										...
										/>
										{templateSearch && (
											<button onClick={() => setTemplateSearch("")} className="cursor-pointer text-slate-400 hover:text-slate-600">
												<X className="h-3.5 w-3.5" />
											</button>
										)}
									</div>
									<Button variant="outline" size="sm" className="h-10 border-slate-200 bg-white shadow-sm px-3 text-slate-600 hover:bg-slate-50 cursor-pointer shrink-0">
								...
								<div className="border border-slate-100 rounded-lg overflow-hidden shadow-sm">
									{updateTemplatesQuery.isLoading ? (
										...
										<div className="px-4 py-10 text-center text-[13.5px] text-slate-400">
										...
										filteredTemplates.map((t) => (
											<button
												...
												className="w-full flex items-center justify-between px-4 py-3 hover:bg-slate-50 transition-colors cursor-pointer border-b border-slate-100 last:border-0"
											>
												<div className="flex items-center gap-3">
													<TemplateIcon type={(t as any).entityType || "TASK"} />
													<span className="text-[14px] text-slate-700">{t.name}</span>
												</div>
												<div
													className={cn(
					...
					<div className={cn("p-5 px-8 flex justify-between items-center border-t border-slate-100 bg-white shrink-0", showUpdateEmptyState && "hidden")}>
						<button
							onClick={() => { setView("save"); setSelectedTemplateId(null); }}
			...
			<Dialog open={open} onOpenChange={onOpenChange}>
				...
				>
					<div className="px-6 py-4 border-b bg-white flex shrink-0 items-center justify-center relative">
						...
						<button
							onClick={() => onOpenChange(false)}
							className="absolute right-4 size-7 flex items-center justify-center rounded-md text-slate-400 hover:text-slate-600 hover:bg-slate-100 transition-colors cursor-pointer"
						>
							<X className="size-4" />
						</button>
					</div>
					...
					<div className="p-5 px-8 flex justify-between items-center border-t bg-white shrink-0">
						<button
							...
							className="flex items-center gap-2 text-[13px] font-medium text-slate-600 hover:text-slate-900 transition-colors cursor-pointer"
						>
