...
import { Loader2, Users, Building2, ChevronRight, CheckCircle2 } from "lucide-react";
...
export function TeamImportModal({ spaceId, open, onOpenChange }: TeamImportModalProps) {
    ...
    const handleImport = async () => {
        ...
        try {
            ...
            toast({ title: `${selectedIds.length} team${selectedIds.length > 1 ? "s" : ""} imported` });
    ...
    const toggleAll = () =>
        setSelectedIds(allSelected ? [] : teams.map(t => t.id));
            ...
            <DialogContent className="sm:max-w-[480px] p-0 gap-0 overflow-hidden">
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <DialogDescription className="text-xs text-slate-500 mt-0.5 leading-relaxed">
                                Move unassigned teams into a workspace space to organize them.
                            </DialogDescription>
                        </div>
                    </div>
                </div>
                    ...
                    <div className="space-y-1.5">
                                ...
                                {isLoading ? (
                                    ...
                                    <div className="flex flex-col items-center justify-center gap-2 h-52 p-4">
                                        ...
                                        <div className="text-center">
                                            <p className="text-xs font-medium text-slate-700">All teams are assigned</p>
                                            <p className="text-xs text-slate-400 mt-0.5">No unassigned teams to import</p>
                                        </div>
                                    </div>
                                ) : (
                        ...
                        {selectedIds.length > 0 && (
                            <p className="text-xs text-slate-500">
                                <span className="font-medium text-slate-800">{selectedIds.length}</span> team{selectedIds.length > 1 ? "s" : ""} selected
                                {targetSpace && (
                ...
                <div className="flex items-center justify-between gap-3 px-6 py-4 border-t border-slate-100 bg-slate-50/50">
                    ...
                    <Button
                        ...
                        disabled={selectedIds.length === 0 || !selectedSpaceId || updateTeam.isPending}
                    ...
                    >
                        {updateTeam.isPending ? (
                            ...
                            <>Import {selectedIds.length > 0 ? `${selectedIds.length} ` : ""}team{selectedIds.length !== 1 ? "s" : ""}</>
                        )}
                    </Button>
                </div>
            </DialogContent>
