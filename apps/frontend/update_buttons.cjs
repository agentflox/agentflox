...
for (const file of files) {
  ...
  // Replace within PageHeader actions
  ...
  if (match) {
    ...
    // Skip if already has group-hover:rotate-90 we already updated it (tasks)
    ...
    // Extract Text (usually "New [Entity]")
    const textMatch = oldButton.match(/(?:>| )New\s+\w+/i);
